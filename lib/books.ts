...
export function getAllReading(): Book[] { return JSON.parse(fs.readFileSync(path.join(process.cwd(), "content", "books", "index.json"), "utf8")); }
export function getAllSlugs(): string[] { const data = getAllReading(); return data.map((item) => item.slug); }
