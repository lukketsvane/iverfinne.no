...
export function Bookshelf({books}: BookshelfProps) {
 ...
 const width = 41.5, height = 220, spineWidth = `${width}px`, coverWidth = `${width * 4}px`, bookWidth = `${width * 5}px`, bookHeight = `${height}px`;
 const minScroll = 0;
 ...
 const boundedRelativeScroll = useCallback((incrementX: number) => setScroll((_scroll) => Math.max(minScroll, Math.min(maxScroll, _scroll + incrementX))), [maxScroll]);
 useEffect(() => {
 ...
 useEffect(() => {
   if (bookIndex === -1) boundedRelativeScroll(0);
   else boundedScroll((bookIndex - (booksInViewport - 4.5) / 2) * (width + 11));
 }, [bookIndex, boundedRelativeScroll]);
 useEffect(() => {
   if (viewportDimensions) {
     boundedRelativeScroll(0);
     setBooksInViewport(viewportDimensions.contentBox.width / (width + 11));
   }
