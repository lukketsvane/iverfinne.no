...
export function Bookshelf({ books }: BookshelfProps) {
  ...
  const maxScroll = React.useMemo(
    () => (width + 12) * (books.length - booksInViewport) + (bookIndex > -1 ? width * 4 : 0) + 5,
  ...
  const boundedRelativeScroll = React.useCallback(
    (incrementX: number) => {
      setScroll((_scroll) => Math.max(minScroll, Math.min(maxScroll, _scroll + incrementX)));
    },
    [maxScroll, minScroll]
  );
  ...
  }, [bookIndex, boundedRelativeScroll, booksInViewport, width]);
  ...
  }, [viewportDimensions, boundedRelativeScroll, width]);
  ...
  return (
    <>
      ...
      <Box position="relative" ref={bookshelfRef}>
        <Box position="absolute" left={{ base: '-28px', md: '-36px' }} height="100%" display={scroll > minScroll ? 'block' : 'none'}>
          <Center
            ref={scrollLeftRef}
            borderRadius="md"
            height="100%"
          ...
          {books.map((book, index) => (
            ...
            >
              ...
              >
                ...
                />
                <Heading mt="12px" as="h2" fontSize="xs" fontFamily={`"DM Sans", sans-serif`} userSelect="none" textOverflow="ellipsis" whiteSpace="nowrap" overflow="hidden" maxHeight={`${height - 24}px`}>
                  {book.title}
                </Heading>
              </Flex>
              <Box
                ...
                transform={`translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(${bookIndex === index ? '30deg' : '88.8deg'}) rotateZ(0deg) skew(0deg, 0deg)`}
                transition={'all 500ms ease'}
                willChange="auto"
                filter="brightness(0.8) contrast(2)"
                style={{ transformStyle: 'preserve-3d' }}
